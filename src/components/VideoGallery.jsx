...
const VideoGallery = ({ isListView = false }) => {
  ...
  //   }`,
  // });
  const [items, setItems] = useState(videos.items);
  // const [videos, setVideos] = useState({
  //   items: [],
  //   isLoading: true,
  //   erroeMessage: "",
  // });
  // const [nextPageToken, setNextPageToken] = useState(videos.nextPageToken);
  ...
  const fetchMoreData = () => {
    ...
    //   isLoading,
    // } = useAxios({
    //   url: `https://youtube.googleapis.com/youtube/v3/videos?part=snippet,statistics&chart=mostPopular&maxResults=10&pageToken=${nextPageToken}&key=${
    //     import.meta.env.VITE_GOOGLE_API_KEY
    //   }`,
    // });
    setItems((prevItems) => [...prevItems, ...videos.items]);
  };
